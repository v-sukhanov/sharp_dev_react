	...
	return (
		<div>
			<div className="search mb-5">
				...
				<input onChange={(event: ChangeEvent<HTMLInputElement>) => setSearch(event.target.value)} type="text" placeholder="Найти пользователя..." />
			</div>
			<li>
				{
					users.filter(user => {
						...
						return (
							<div key={user.id} className="flex justify-between items-center gap-5 mb-5">
									...
									<span onClick={() => handleAddOrDeleteInvites(user.id)} className="transition hover:text-black text-gray-400 text-4xl cursor-pointer">
			...
			<button className="px-5 py-2 rounded bg-blue-500 w-full mt-5 text-white">Отправить приглашение</button>
